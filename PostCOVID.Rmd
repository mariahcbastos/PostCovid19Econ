...
># Alternative Hypothesis: Hashtag Retweets  for Democrats are equal to Republicans
...
```
># The data set is not normally distributed since p-value is less than alpha, therefore the distribution is significantly different from a normal distribution, we cannot assume normality. 
...
```
# ># P value is smaller than alpha, we reject the null hypothesis. We support the alternative claim. 
...
># P-Value is lower than alpha, therefore we reject the null hypothesis that at all the means are equal and we conclude that at least one of the population means is different. 
